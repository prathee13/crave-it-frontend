    ...
    striped responsive="sm">
      ...
      <template v-slot:row-details="row">
        <b-card>
         ...
         <b-row class="mb-2">
            <b-col  class="text-sm-right"><b>Pickup Location:</b></b-col>
            <b-col>{{ row.item.pickup_address }}</b-col>
          </b-row>
 ...
 import DishAddNotifyService from '../_service/dish.service';
  ...
  export default {
    ...
    methods: {
        ...
        markServed(id) {
            Axios.get(`order/served/${id}`).then(success => {
                this.getOrders(this.selectedStatus)
            }, error => {
                alert("error occured while canceling your order.\nPlease try again!")
            })
        ...
        getOrders(status) {
            Axios.get('order/list/buyer/').then(success => {
                this.items = []
                this.served = []
                this.refunded = []
                this.placed = []
